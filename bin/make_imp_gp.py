...
from geomag_imp import gpKernels, gpRegressor
...
if __name__ == "__main__":
   ...
   for tidx in np.arange(len(obs_Btheta_Bphi_Br)):
      ...
      # pre-allocate predicted output array
      pred_Bt_Bp_Br = np.zeros((pred_lat_lon_r.shape[0], 3))

      for comp in range(3):
         # we could just pass the 3-column input array to the Gaussian process,
         # but if any component was bad, the whole observation would be dropped,
         # or worse, we'd crash, neither of which is desirable.

         # identify good observations for this time step
         good = np.isfinite(obs_sigma_Bt_Bp_Br[:, comp])

         if np.any(good):
            # fit the Gaussian Process
            imp.fit(
               np.hstack((obsY.reshape(-1,1),
                          obsX.reshape(-1,1),
                          obs_lat_lon_r[:,2].reshape(-1,1)))[good],
               obs_Bt_Bp_Br[good, comp]
            )
            # predict on the output grid
            pred_Bt_Bp_Br[:, comp] = imp.predict(
               np.hstack((predY.reshape(-1,1),
                          predX.reshape(-1,1),
                          pred_lat_lon_r[:,2].reshape(-1,1)))
            )
         else:
            # nothing else is possible if there are no good observations
            pred_Bt_Bp_Br[:, comp] = np.zeros(pred_lat_lon_r.shape[0]) * np.nan

         # # copy predicted component to output array
         # pred_Bt_Bp_Br[:, comp] = pred_comp

      # accumulate interpolated output
      # (convert from Tesla to nanoTesla)
      X.append(-pred_Bt_Bp_Br[:,0] * 1e9)
      Y.append(pred_Bt_Bp_Br[:,1] * 1e9)
      Z.append(-pred_Bt_Bp_Br[:,2] * 1e9)
   ...
   if write_CDF:
      ...
      imp_io.write_imp_CDF(
         ...
         filename = "impGP_%s--%s.cdf"%(Epoch[0].isoformat(),
                                      Epoch[-1].isoformat())
      )
   ...
   if write_JSON:
      ...
      imp_io.write_imp_JSON(
         ...
         (coord for coord in pred_lat_lon_r.T), X, Y, Z, Label,
         (coord for coord in obs_lat_lon_r.T), ObsX, ObsY, ObsZ, ObsFit, ObsName,
         filename = "impGP_%s--%s.json"%(Epoch[0].isoformat(),
                                      Epoch[-1].isoformat())
      )
   ...
   if write_ASCII:
      ...
      imp_io.write_imp_ASCII(
         ...
         (coord for coord in pred_lat_lon_r.T), X, Y, Z, Label,
         (coord for coord in obs_lat_lon_r.T), ObsX, ObsY, ObsZ, ObsFit, ObsName,
         filename = "impGP_%s--%s.zip"%(Epoch[0].isoformat(),
                                      Epoch[-1].isoformat())
      )
